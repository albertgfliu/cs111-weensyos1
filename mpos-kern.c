...
start(void)
{
	...
	//SELF ADDITION: Mark the original starting process (pid 0) as not holding up anything (a.k.a. it holds up process at pid -1)
	current->p_waitproc_pid = -1;
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		//SELF ADDITION EX 3: process is exiting, no longer blocking anything
		if(current->p_waitproc_pid > 0){
			proc_array[current->p_waitproc_pid].p_state = P_RUNNABLE; //set the blocked process to runnable
			proc_array[current->p_waitproc_pid].p_registers.reg_eax = current->p_exit_status; //update syscall return value
		}
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			proc_array[p].p_state = P_EMPTY; //SELF ADDITION EX 4: set empty state if it's a zombie. Wasn't in skeleton code, suggested by TA to add.
			current->p_registers.reg_eax = proc_array[p].p_exit_status;
		}
		else {
			current->p_registers.reg_eax = WAIT_TRYAGAIN;
			
			//SELF ADDITION EX 3: add blocking
			current->p_state = P_BLOCKED;
			proc_array[p].p_waitproc_pid = current->p_pid;
		}
		schedule();
	}
...
do_fork(process_t *parent)
{
  ...
  pid_t curr_index;
  pid_t first_empty_index;
  char empty_found;
  process_t* new_process;

  for(curr_index = NPROCS-1; curr_index > 0; curr_index--){ //don't use 1, it is the original parent process
	if(proc_array[curr_index].p_state == P_EMPTY){
		empty_found = 1;
		first_empty_index = curr_index;
		new_process = &proc_array[curr_index];
	}
  }
...
copy_stack(process_t *dest, process_t *src)
{
	...
	src_stack_top = PROC1_STACK_ADDR + (src->p_pid)*PROC_STACK_SIZE /*mycode*/;
	src_stack_bottom = src->p_registers.reg_esp;
	dest_stack_top = PROC1_STACK_ADDR + (dest->p_pid)*PROC_STACK_SIZE /*mycode*/;
	dest_stack_bottom = dest_stack_top + (src_stack_bottom - src_stack_top) /*mycode, calculate based on src_stack size and top of dest stack */;
